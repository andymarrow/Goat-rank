...
export default function CreateClient({ categories }: { categories: string[] }) {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto bg-card border border-border/80 rounded-3xl shadow-2xl relative overflow-hidden min-h-0 md:min-h-[600px] flex flex-col font-sans">

      <div className="absolute top-0 right-0 w-96 h-96 bg-primary/10 blur-[100px] rounded-full pointer-events-none" />

      <div className="flex items-center justify-between p-6 border-b border-border/80 z-10">
        <h2 className="text-lg sm:text-xl font-extrabold text-foreground tracking-tight uppercase">HOST BATTLE</h2>
        <div className="hidden md:flex items-center gap-2 text-xs font-bold text-muted-foreground uppercase tracking-wider">
          <span className={step >= 1 ? "text-primary font-extrabold" : ""}>01. BRIEFING</span>
          <span className="opacity-30">/</span>
          <span className={step >= 2 ? "text-primary font-extrabold" : ""}>02. LOADOUT</span>
          <span className="opacity-30">/</span>
          <span className={step >= 3 ? "text-primary font-extrabold" : ""}>03. DEPLOY</span>
        </div>
        <div className="md:hidden text-xs font-bold text-primary uppercase tracking-wider">STEP 0{step}/03</div>
      </div>
